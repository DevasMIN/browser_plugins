...
- **0.9.26** — число случайных видео стало настраиваемым (степпер −/+,
  ...
  этого блока.
- **0.9.27** — фильтр «скрывать Shorts» (включён по умолчанию): YouTube не
  помечает Shorts в плейлисте загрузок никак отдельно от обычных видео, поэтому
  фильтр прячет всё короче 3 минут.
