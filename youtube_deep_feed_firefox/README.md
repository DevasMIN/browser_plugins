...
- **0.9.26** — число случайных видео стало настраиваемым (степпер −/+,
  ...
  этого блока.
- **0.9.27** — Shorts больше не учитываются вообще: YouTube не помечает их
  никак отдельно в плейлисте загрузок, только длительностью (до 3 минут), —
  всё короче 3 минут не сохраняется в базу и не попадает ни в ленту, ни в
  подсчёт непросмотренных, без переключателя.
