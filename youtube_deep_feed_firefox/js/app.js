 ...
 * Обход всех каналов сразу занимал минуты и блокировал показ новых видео —
 ...
 * ещё не до конца просканированных каналов. Полный обход всех каналов разом
 * (как раньше делал quickSync каждые 6 часов) занимал минуты и всё это время
 * не показывал уже найденные новые видео — с ротацией каждый тик короткий,
 * а найденное видно сразу (announceNewVideos).
 */
 ...
 * (runTask сам делает resetFeed), чтобы не дёргать DOM много раз подряд.
 */
...
/** Полностью очищает #feed вместе со связанным состоянием превью (ленивая
 * загрузка/blob-кэш) — общая часть между resetFeed() и renderShuffle(). */
