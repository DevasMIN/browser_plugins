...
export async function getConfig() {
  ...
  const stored = await browserAPI.storage.local.get('itCfg');
  ...
  await browserAPI.storage.local.set({ itCfg: cachedCfg });
  return cachedCfg;
}
...
async function sapisidHash() {
  const cookie =
    (await browserAPI.cookies.get({ url: YT, name: 'SAPISID' })) ||
    (await browserAPI.cookies.get({ url: YT, name: '__Secure-3PAPISID' }));
...
async function pingTab(tabId) {
  try {
    const resp = await browserAPI.tabs.sendMessage(tabId, { type: 'ping' });
    return !!(resp && resp.ok);
  } catch (e) {
    return false;
  }
...
async function getStoredRelayTab() {
  try {
    const s = await browserAPI.storage.session.get('relayTabId');
...
async function setStoredRelayTab(id) {
  try {
    await browserAPI.storage.session.set({ relayTabId: id });
  } catch (e) {
    /* ignore */
  }
}
 ...
 * Нужен только как запасной путь, если переписывание Origin через webRequest
 * (см. background.js) не помогло и прямой запрос всё равно даёт 403.
 * Дедуплицируется: одну вкладку переиспользуем
...
async function ensureRelayTab() {
  ...
  for (const tab of await browserAPI.tabs.query({ url: '*://www.youtube.com/*' })) {
  ...
  const tab = await browserAPI.tabs.create({ url: `${YT}/?hl=ru`, pinned: false, active: false });
  for (let i = 0; i < 50; i++) {
    await sleep(400);
    if (await pingTab(tab.id)) {
      relayTabId = tab.id;
...
async function relayApi(endpoint, body) {
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    let resp;
    try {
      resp = await browserAPI.tabs.sendMessage(tabId, { type: 'api', endpoint, body });
    } catch (e) {
 ...
 * `//` резолвится в `moz-extension://…` и картинка не грузится.
 */
...
const UNIT_MS = [
  ...
  // Без \b: в JS граница слова считается по ASCII-\w, поэтому «\bдн» никогда
  // не совпадает с кириллицей — «2 дня»/«5 дней» переставали распознаваться
  // (парсился только вариант «1 день»), и окно ленты схлопывалось в 1 день.
  [/дн|день|day/i, 86400e3, 86400e3],
...
function matchRelativeDate(text) {
  ...
  // Строка метаданных иногда приходит склеенной с просмотрами в одно поле
  // («300 тыс. просмотров • 4 дня назад») — берём число вплотную перед
  // «назад»/«ago», а не первое попавшееся (иначе цепляем счётчик просмотров).
  const m = t.match(/(\d+)\s*(\S+)\s*(?:назад|ago)/i);
  if (!m) return null;
